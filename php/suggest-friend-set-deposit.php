...
if ($result->num_rows > 0)
{
  ...
  $sql = "SELECT member_nickname, member_id FROM backend_member_account WHERE member_sbobet_account_id = $member_sbobet_account_id";
  $result = $conn->query($sql);
  if ($result->num_rows > 0)
  {
    while($row = $result->fetch_assoc())
    ...
    VALUES ('$account', '$member_nickname', '$tel', '$deposit_amount', '0', '0', '163', 'promotion', 'refer', '$date', '$time', '$timeStamp', '5', null, null, 'ยอดเงินได้รับฟรีจากโปรโมชั่นแนะนำเพื่อน', 'No', 'No', 'No')";

    if ($conn->query($sql) === TRUE) {
      $last_id = $conn->insert_id;
      $add_result = json_decode(add_credit($last_id,$account,'promotion',$deposit_amount), true);
