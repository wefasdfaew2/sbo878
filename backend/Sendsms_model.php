...
class Sendsms_model extends CI_Model {
    ...
    public function sendsms($sms_telephone,$sms_message,$sms_flag){
      ...
      //if mobile_carrier_lookup=true then SEND BY diafaan
      	if($this->check_carrier($sms_telephone)=='true'){
      	 $sms_message_cast = str_replace(' ', '%20', $sms_message);
      		$this->send_sms_diafaan($sms_telephone,$sms_message_cast,'9711');
      		//echo "send_sms_diafaan";
      	}else{//else SEND BY tmtopup
      		$this->send_sms_tmtopup($sms_telephone,$sms_message);
      	...
      	//also send thaibluk
      	   $this->send_sms_thaibulksms($sms_telephone,$sms_message);
      ...
      if($this->check_carrier($sms_telephone)=='true'){
      		...
      		$sms_message_cast = str_replace(' ', '%20', $sms_message);
      		 $this->send_sms_diafaan($sms_telephone,$sms_message_cast,'9710');
    ...
    public function send_sms_thaibulksms($sms_telephone,$sms_message){
    ...
    echo $conn2->error;
        ...
        $data_sms_log = array(
           'sms_log_type' => $sms_log_type ,
           'sms_log_from' => $sms_log_from ,
           'sms_log_to' => $sms_log_to ,
           'sms_log_msg' => $sms_log_msg ,
           'sms_log_status' => $sms_log_status ,
           'sms_log_transaction' => $sms_log_transaction ,
           'sms_log_dn_msg' => $sms_log_dn_msg  ,
           'sms_log_time' => $sms_log_time
        );
    $this->db->insert('sms_log', $data_sms_log);

    return $status_send_sms;

    }

    public function check_carrier($sms_telephone){
     ...
     // Create connection
      ...
      if ($conn->connect_error)
      {
          die("Connection failed: " . $conn->connect_error);
    		...
    		$sql_carrier = "SELECT `mobile_carrier`	FROM `mobile_carrier_lookup`
          ...
          $sql = "SELECT mobile_carrier FROM mobile_carrier_lookup WHERE member_telephone = $sms_telephone ORDER BY 'id' DESC LIMIT 1";
          $query = $this->db->query($sql)->row();

          /*$db_carrier = $this->db->select('mobile_carrier')
            //->from('mobile_carrier_lookup')
            ->where('mobile_carrier_lookup.member_telephone', $sms_telephone)
            ->order_by("id", "desc")
            ->limit(1)
            ->get('mobile_carrier_lookup')
            ->row();*/
      //  error_log($query->mobile_carrier);

    		return $query->mobile_carrier;
    }

    public function send_sms_tmtopup($sms_telephone,$sms_message){
    // กำหนด UID ของท่าน
    /*
    ...
    */
    ...
    public function send_sms_diafaan($sms_telephone,$sms_message,$port){
